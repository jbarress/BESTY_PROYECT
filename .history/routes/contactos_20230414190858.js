...
app.post('/', async (req, res) => {
    const { title, start, end } = req.body; // Obtén el título y la fecha del cuerpo de la solicitud
    ...
    try {
        ...
        console.log(error); 
        res.status(500).send({ error: `Error al guardar el evento: ${error}` }); // Devuelve un error si no se pudo guardar el evento
    }
});
module.exports = app;