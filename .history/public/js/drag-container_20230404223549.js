...
var offsetX, offsetY, offsetXC, offsetYC;
...
container.addEventListener("mousedown", function(e) {
  ...
  document.addEventListener("mousemove", mousemoveHandlerC);
});

document.addEventListener("mouseup", function(e) {
  document.removeEventListener("mousemove", mousemoveHandlerC);

  // Guardamos la posición actual del contenedor en el almacenamiento local
  var currentPositionC = {
    left: calendar.style.left,
    top: calendar.style.top
  };
  localStorage.setItem("containerPositionC", JSON.stringify(currentPositionC));
});

function mousemoveHandlerC(e) {
  var x = e.clientX - offsetXC;
  var y = e.clientY - offsetYC;
